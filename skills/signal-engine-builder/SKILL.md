...
- Fit and select models with episode-independent evidence. Aggregate features to a justified causal episode anchor or use episode-balanced weighting so each approved or matched-neutral episode contributes total weight one. Never let a long bracket contribute hundreds of effective training examples. Build internal chronological blocks from complete episodes and purge or embargo boundaries by the target outcome horizon so overlapping label windows cannot cross train/evaluation boundaries.
...
- Fitting and chronological validation use episode-independent evidence: each approved or matched-neutral episode has total weight one, complete episodes remain in one block, and the target outcome horizon is purged or embargoed at boundaries.
