...
DEFAULT_RESET_LOOKBACK_BARS = 72
DEFAULT_MIN_RESET_AGE_BARS = 1
DEFAULT_MAX_RESET_AGE_BARS = 72
DEFAULT_MIN_FLUSH_EXTENSION_PCT = 0.05
DEFAULT_MIN_FLUSH_CANDLE_RANGE_PCT = 0.25
DEFAULT_MIN_RESET_OI_DROP_1H_PCT = 0.02
DEFAULT_MIN_RESET_VOLUME_RATIO = 1.00
DEFAULT_MIN_RECLAIM_FROM_EXTREME_PCT = 0.08
DEFAULT_MIN_OI_REBUILD_1H_PCT = 0.005
DEFAULT_MIN_VOLUME_RATIO = 0.95
DEFAULT_MIN_TAKER_IMBALANCE_Z = 0.05
DEFAULT_TAKER_BUY_RATIO_THRESHOLD = 1.01
DEFAULT_TAKER_SELL_RATIO_THRESHOLD = 0.99
DEFAULT_MAX_ABS_FUNDING_ZSCORE = 2.10
DEFAULT_MAX_ABS_PREMIUM_ZSCORE = 2.10
DEFAULT_POST_RESET_GLOBAL_RATIO_LOW = 0.70
DEFAULT_POST_RESET_GLOBAL_RATIO_HIGH = 1.35
    ...
    *,
    ...
    for anchor_index in range(start, end + 1):
        ...
        if not price_impulse_ok:
            continue
        extension_ok = flush_extension_pct >= float(parameters["min_flush_extension_pct"])
        range_ok = flush_candle_range_pct >= float(parameters["min_flush_candle_range_pct"])
        if not extension_ok and not range_ok:
            continue
