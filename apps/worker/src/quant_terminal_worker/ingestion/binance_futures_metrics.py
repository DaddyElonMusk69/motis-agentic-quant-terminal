...
def build_live_futures_metrics_rows(
    *,
    ...
    # Binance history endpoints can behave as end-time exclusive. Snapshot
    # endpoints report the 5m interval by its interval-end timestamp, so ask one
    # extra raw interval beyond the last wanted snapshot timestamp and then only
    # emit rows through ``end`` below.
    snapshot_end = end + BASE_INTERVAL * 2
    common_snapshot_kwargs: dict[str, Any] = {
        "symbol": symbol,
        "period": TIMEFRAME,
        "limit": limit,
    }
        ...
        taker_kwargs.update(
            {
                ...
                "end_time_ms": _to_epoch_ms(end + BASE_INTERVAL),
            }
        )
    ...
    *,
    ...
    if target - page_start > rolling_window:
        source_dir = archive_source_dir or (
            Path(".data/downloads/binance/futures_metrics") / symbol
        )
        ...
        if archive_end_date >= page_start.date():
            downloader = archive_downloader or download_binance_futures_metrics_archives
            downloader(
    ...
    if page_start <= target:
        live_rows = build_live_futures_metrics_rows(
            adapter=adapter,
            symbol=symbol,
            from_ts=page_start,
            target=target,
    ...
    if stored_row_count < previous_row_count:
        raise RuntimeError(
            f"stored futures metrics row count regressed for {registration['dataset_id']}: "
    ...
    quality = _increment_quality_summary(
        ...
        added_complete_rows=registered_rows_added,
    )
    updated = {
        **registration,
    ...
    derived_rebuilt = _refresh_derived_refs_incrementally(
        ...
        first_changed_ts=_coerce_datetime(new_rows[0]["timestamp"]),
        last_changed_ts=_coerce_datetime(new_rows[-1]["timestamp"]),
        repository=repository,
    )
    final_end_ts = _coerce_datetime(new_rows[-1]["timestamp"])
    ...
    result = {
        ...
        "to_ts": _to_iso(target),
        "source": (
            "binance_archive_and_cli" if archive_row_count else "binance_cli"
        ),
...
def _contiguous_rows_from(
    *,
    rows: Iterable[dict[str, Any]],
    start: datetime,
) -> list[dict[str, Any]]:
    expected = _floor_to_5m_boundary(_coerce_datetime(start))
    output: list[dict[str, Any]] = []
...
def _read_archive_rows(
    *,
    ...
    for path in sorted(source_dir.rglob(f"{symbol.upper()}-metrics-*.zip")):
        archive_day = _metrics_archive_day(path, symbol=symbol)
        if archive_day is not None:
            if minimum is not None and archive_day < minimum.date():
                continue
            if maximum is not None and archive_day > maximum.date():
                continue
        with zipfile.ZipFile(path) as archive:
            csv_name = next(
            ...
            with archive.open(csv_name) as handle:
                ...
                for raw_row in csv.DictReader(text):
                    row = normalize_archive_futures_metrics_row(raw_row)
                    timestamp = _coerce_datetime(row["timestamp"])
...
def _metrics_archive_day(path: Path, *, symbol: str) -> date | None:
    prefix = f"{symbol.upper()}-metrics-"
    name = path.name
    if not name.startswith(prefix) or not name.endswith(".zip"):
        return None
    try:
        return date.fromisoformat(name[len(prefix) : -4])
    except ValueError:
        return None
...
def _refresh_derived_refs_incrementally(
    *,
    ...
    for registration in registrations:
        ...
        rows = [
            row
            for row in derive_futures_metrics_rows(
                raw_rows=raw_rows,
                raw_timeframe=TIMEFRAME,
                derived_timeframe=timeframe,
            )
